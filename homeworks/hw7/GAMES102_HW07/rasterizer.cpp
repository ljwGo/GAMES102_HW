...
void rst::rasterizer::drawDelTri(const std::vector<Triangle>& tris, const std::vector<Vector2f>& vs, const Vector3f& color)
{
    if (tris.size() >= 1) {
        std::vector<Edge> edges;
        edges.reserve(tris.size() * 3);
        std::vector<bool> flags(tris.size() * 3, true);

        for (int i = 0; i < tris.size() - 1; ++i) {
            for (int j = i + 1; j < tris.size(); ++j) {
                int ix = tris[i].PublicEdge(tris[j]);
                if (ix != -1) flags[i * 3 + ix] = false;
            }
        }
        ...
        for (int i = 0; i < tris.size(); ++i) {
            for (int j = 0; j < 3; ++j) {
                if (flags[i * 3 + j]) {
                    edges.push_back(tris[i].edges[j]);
                }
            }
        }

        for (const Edge& e : edges) {
            draw_line(vs[e.vsInd], vs[e.veInd], color);
        }
    }
}

void rst::rasterizer::drawTriangle(const Triangle& tris)
{
    draw_line(tris.v1(), tris.v2());
    draw_line(tris.v2(), tris.v3());
    draw_line(tris.v3(), tris.v1());
}
